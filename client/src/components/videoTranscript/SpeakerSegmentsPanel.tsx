...
export default function SpeakerSegmentsPanel(props: {
  ...
  const speakerColorIdx = (rawName: string) =>
    uniqueSpeakers.indexOf(rawName) % speakerColors.length;
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="flex items-center gap-2 mb-1 shrink-0">
        ...
        {audioObjectUrl && (
          <span className="ml-auto text-[11px] font-normal text-gray-400 dark:text-gray-500 shrink-0">
            Tap a line to seek
          </span>
        )}
      </div>
      ...
      {showMapPanel && canRename && (
        <div className="mb-4 rounded-xl border border-blue-200 dark:border-blue-800 bg-blue-50/60 dark:bg-blue-950/20 p-3 shrink-0">
          <p className="text-[11px] font-medium uppercase tracking-wide text-blue-700 dark:text-blue-400 mb-3">
            Name each speaker
          </p>
            ...
            {uniqueSpeakers.map((raw, i) => {
              ...
              return (
                <div key={raw} className="flex items-center gap-2">
                  {/* Current auto-label */}
                  <span
                    className={`text-[11px] font-medium uppercase w-20 shrink-0 truncate ${textColor}`}
                  >
                    {currentNameOf(raw)}
                  </span>
                  ...
                  <input
                    ...
                    value={drafts[raw] ?? ""}
                    onChange={(e) =>
                      setDrafts((prev) => ({ ...prev, [raw]: e.target.value }))
                    }
                    onKeyDown={(e) => {
      ...
      <div className="space-y-2 flex-1 min-h-0 overflow-y-auto pr-1">
        {data.map((item, i) => {
          ...
          const textColorClass =
            speakerTextColors[speakerColorIdx(item.rawSpeaker)];
          const ts = seg
          ...
          return (
            ...
            >
              <div className="shrink-0 flex flex-col items-end gap-0.5 pt-0.5 w-16">
                <span
                  className={`text-[11px] font-medium uppercase truncate max-w-full ${
                    isActive
                      ? textColorClass
                ...
                {ts && (
                  <span
                    className={`text-[10px] font-mono ${isActive ? textColorClass + " opacity-70" : "text-gray-300 dark:text-gray-600"}`}
                  >
                    {ts}
                  </span>
                )}
              ...
              <p
                className={`flex-1 text-sm leading-relaxed ${isActive ? "text-gray-900 dark:text-gray-100 font-medium" : "text-gray-600 dark:text-gray-300"}`}
              >
                {isActive && (
                  <span
                    className="inline-block w-1.5 h-1.5 rounded-full bg-blue-600 mr-1.5 mb-0.5 animate-pulse"
                    aria-hidden
                  />
                )}
