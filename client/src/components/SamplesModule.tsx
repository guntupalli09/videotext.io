...
export default function SamplesModule({ sourcePath, samplesHref = '/samples' }: SamplesModuleProps) {
  ...
  return (
    <div className="rounded-2xl border border-violet-200/70 dark:border-violet-800/50 bg-violet-50/70 dark:bg-violet-950/20 px-4 py-4 sm:px-6 sm:py-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-start gap-3">
          <FlaskConical className="h-5 w-5 mt-0.5 text-violet-600 dark:text-violet-300" />
          <div>
        ...
        <Link
          to={samplesHref}
          onClick={handleClick}
          className="inline-flex items-center gap-1.5 rounded-lg bg-violet-600 hover:bg-violet-700 text-white text-sm font-semibold px-3 py-2 transition-colors"
        >
