    ...
    <div className="min-h-screen bg-white dark:bg-gray-900">
      ...
      <div className="bg-gradient-to-b from-blue-50 to-white dark:from-gray-800 dark:to-gray-900 border-b border-gray-100 dark:border-gray-700">
        <div className="max-w-3xl mx-auto px-4 py-10 sm:py-14 text-center">
          {hubLink && (
            <Link to={resolveInternalLinkPath(hubLink.path)} className="inline-flex items-center gap-1.5 text-xs font-medium text-blue-600 dark:text-blue-400 hover:underline mb-3">
              <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
              {hubLink.label}
            </Link>
          )}
          {!hubLink && (
            <span className="inline-block text-xs font-medium uppercase tracking-widest text-blue-600 dark:text-blue-400 mb-3">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-12">
        {isSubtitleCluster && (
          ...
          >
            ...
            <p className="text-sm text-gray-700 dark:text-gray-300 mb-3">
              ...
              <Link to="/subtitle-tools" className="font-medium text-blue-700 dark:text-blue-400 underline underline-offset-2">
            ...
            <Link
              to="/subtitle-tools"
              className="inline-flex items-center justify-center rounded-xl bg-blue-600 hover:bg-blue-600 text-white text-sm font-medium px-4 py-2.5 transition-colors"
            >
        ...
        <section className="rounded-xl bg-gray-900 dark:bg-gray-800 border border-gray-700 dark:border-gray-600 overflow-hidden">
          <div className="px-6 py-7 sm:px-8">
            <p className="text-xs font-medium uppercase tracking-widest text-blue-400 mb-2">⚡ Skip the manual steps</p>
            ...
            <ul className="space-y-2 mb-6">
              {[
                ...
                <li key={text} className="flex items-start gap-2.5 text-sm text-gray-300">
                  <span className="text-green-400 font-medium shrink-0 mt-px">{text.slice(0, 1)}</span>
                  {text.slice(2)}
                </li>
              ))}
            </ul>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <Link
                ...
                className="inline-flex items-center justify-center rounded-xl bg-blue-600 hover:bg-blue-600 text-white font-medium px-6 py-3 text-sm transition-colors"
              >
                Try AI Transcription — free →
              </Link>
              <Link
                to="/video-to-subtitles"
                className="inline-flex items-center justify-center rounded-xl border border-gray-600 hover:border-gray-400 text-gray-300 hover:text-white font-medium px-6 py-3 text-sm transition-colors"
              >
                Generate subtitles
              </Link>
            </div>
            ...
            <ol className="space-y-4">
              {guideSteps.map((s, i) => (
                <li key={i} className="flex gap-4">
                  <span className="flex-shrink-0 w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 font-medium text-sm flex items-center justify-center">
                    {i + 1}
                  </span>
                  <div>
                    <p className="font-medium text-gray-900 dark:text-white text-sm">{s.step}</p>
            ...
            <dl className="space-y-5">
              {faqs.map((f, i) => (
                <div key={i} className="border-b border-gray-100 dark:border-gray-700 pb-5 last:border-0 last:pb-0">
                  <dt className="font-medium text-gray-900 dark:text-white text-sm mb-1.5">{f.q}</dt>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {relatedTools.map((t) => (
              <Link
                ...
                className="block rounded-xl border border-gray-200 dark:border-gray-700 p-4 hover:border-blue-300 dark:hover:border-blue-600 hover:bg-blue-50/50 dark:hover:bg-blue-900/10 transition-colors group"
              >
                <p className="font-medium text-sm text-blue-600 dark:text-blue-400 group-hover:underline">{t.label}</p>
          ...
          {hubLink && (
            <div className="mt-3">
              <Link
                ...
                className="block rounded-xl border-2 border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/20 p-4 text-center hover:border-blue-400 transition-colors"
              >
                <p className="font-medium text-sm text-blue-700 dark:text-blue-300">← Back to {hubLink.label}</p>
