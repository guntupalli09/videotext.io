    ...
    <div className="min-h-screen bg-white dark:bg-gray-950 transition-colors duration-500">
      ...
      <section className="relative py-16 sm:py-20 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6">
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200/60 dark:border-emerald-500/20 mb-5">
            <span className="text-[12px] font-medium text-emerald-600 dark:text-emerald-400 uppercase tracking-wide">Open — Transparency report</span>
          </div>
      ...
      <div className="max-w-4xl mx-auto px-6 pb-24 space-y-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {STATS.map(({ icon: Icon, label, value, note }) => (
              <div key={label} className="bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-white/[0.06] p-6 transition-colors duration-500">
                ...
                <div className="text-3xl font-medium text-gray-900 dark:text-white mb-1">{value}</div>
                <div className="text-sm font-medium text-gray-700 dark:text-white/70 mb-1">{label}</div>
        ...
        <section>
          ...
          <div className="rounded-xl border border-gray-200 dark:border-white/[0.06] overflow-hidden">
            <div className="grid grid-cols-3 bg-gray-50 dark:bg-gray-900 px-5 py-3 border-b border-gray-200 dark:border-white/[0.05]">
              <div className="col-span-2 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Audio condition</div>
              <div className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide text-right">Word accuracy</div>
            </div>
            <div className="divide-y divide-gray-100 dark:divide-white/[0.03] bg-white dark:bg-gray-900/50">
              {ACCURACY_DATA.map(({ condition, accuracy }) => (
                <div key={condition} className="grid grid-cols-3 px-5 py-3.5 items-center">
                  ...
                  <span className="text-sm font-medium text-blue-600 dark:text-blue-400 text-right">{accuracy}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <section>
          ...
          <div className="rounded-xl border border-gray-200 dark:border-white/[0.06] overflow-hidden">
            <div className="grid grid-cols-3 bg-gray-50 dark:bg-gray-900 px-5 py-3 border-b border-gray-200 dark:border-white/[0.05]">
              <div className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Video length</div>
              <div className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide text-center">P50 (median)</div>
              <div className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide text-right">P90</div>
            </div>
            <div className="divide-y divide-gray-100 dark:divide-white/[0.03] bg-white dark:bg-gray-900/50">
              {SPEED_DATA.map(({ duration, p50, p90 }) => (
                <div key={duration} className="grid grid-cols-3 px-5 py-3.5 items-center">
                  ...
                  <span className="text-sm font-medium text-emerald-600 dark:text-emerald-400 text-center">{p50}</span>
            ...
            <div className="divide-y divide-gray-100 dark:divide-white/[0.03] bg-white dark:bg-gray-900/50">
              {TECH_STACK.map(({ layer, detail }) => (
                <div key={layer} className="grid grid-cols-2 px-5 py-3.5 items-center">
                  <span className="text-sm font-medium text-gray-700 dark:text-white/70">{layer}</span>
          ...
          <ul className="space-y-3">
            {[
              ...
              <li key={point} className="flex items-start gap-3 text-sm text-emerald-700 dark:text-emerald-300">
                <span className="mt-0.5 text-emerald-500 font-medium flex-shrink-0">✓</span>
                {point}
              </li>
            ))}
          </ul>
