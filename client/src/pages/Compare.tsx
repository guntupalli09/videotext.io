...
function BoolCell({ val, isUs = false }: { val: boolean | string; isUs?: boolean }) {
  ...
  if (typeof val === 'string') {
    return (
      <span className={`text-[12px] font-semibold ${isUs ? 'text-violet-600 dark:text-violet-400' : 'text-gray-600 dark:text-gray-300'}`}>
        {val}
      </span>
    );
  }
...
function SpeedCalculator() {
  ...
  const tools = [
    { name: 'VideoText', minsPerHour: 1.5, color: 'text-violet-600 dark:text-violet-400', bg: 'bg-violet-100 dark:bg-violet-500/15' },
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-white/[0.06] p-6 sm:p-8 transition-colors duration-500">
      ...
      <p className="text-sm text-gray-500 dark:text-white/40 mb-6">
        Estimated AI processing time per video length. VideoText is typically <strong className="text-violet-600 dark:text-violet-400">6–8x faster</strong> than alternatives. Human services (Easyscribe, Go Transcript) take hours to days.
      </p>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        {/* ── VideoText ── */}
        <div className="rounded-2xl border-2 border-violet-300 dark:border-violet-500/40 bg-violet-50/60 dark:bg-violet-500/[0.04] p-5">
          <div className="flex items-center gap-2 mb-4">
            <span className="font-bold text-violet-700 dark:text-violet-400 text-sm">VideoText</span>
          ...
          <div className="space-y-3 text-[13px] leading-relaxed font-mono text-gray-800 dark:text-white/80">
            <p>
              <span className="text-violet-600 dark:text-violet-400 font-bold not-italic">[Sarah Chen · 0:12]</span><br />
              So the thing is — when we launched in 2019, nobody expected the market to shift that fast. Right?
            </p>
            <p>
              <span className="text-violet-600 dark:text-violet-400 font-bold not-italic">[Mike O'Brien · 0:18]</span><br />
              Exactly. And the background noise in that office was insane. You could barely hear yourself think.
            </p>
            <p>
              <span className="text-violet-600 dark:text-violet-400 font-bold not-italic">[Priya Sharma · 0:24]</span><br />
              I remember that. The HVAC system was so loud, we had to stop the recording twice.
            </p>
            <p>
              <span className="text-violet-600 dark:text-violet-400 font-bold not-italic">[Sarah Chen · 0:31]</span><br />
              And still, the results were better than anything we'd seen before.
            </p>
          </div>
          <div className="mt-4 pt-3 border-t border-violet-200 dark:border-violet-500/20 flex items-center gap-1.5">
  ...
  const failures = [
    {
      ...
      good: (
        <div className="font-mono text-[12px] leading-relaxed text-gray-800 dark:text-white/80 space-y-1">
          <p><span className="text-violet-600 dark:text-violet-400 font-bold">[Alex · 1:04]</span> Yeah, no — exactly.</p>
          <p><span className="text-violet-600 dark:text-violet-400 font-bold">[Jamie · 1:06]</span> <span className="italic text-gray-500">[overlapping]</span> So like — I know, right?</p>
          <p><span className="text-violet-600 dark:text-violet-400 font-bold">[Alex · 1:08]</span> So what happened was, we basically had to redo the whole thing from scratch.</p>
    ...
    },
    {
      ...
      good: (
        <div className="font-mono text-[12px] leading-relaxed text-gray-800 dark:text-white/80 space-y-2">
          <p><span className="text-violet-600 dark:text-violet-400 font-bold">[Jordan · 2:10]</span> We started the company in 2020.</p>
          <p><span className="text-violet-600 dark:text-violet-400 font-bold">[Chris · 2:13]</span> Yeah, and it was rough.</p>
          <p><span className="text-violet-600 dark:text-violet-400 font-bold">[Jordan · 2:15]</span> Definitely rough. <span className="italic text-gray-400">[laughter]</span></p>
          <p><span className="text-violet-600 dark:text-violet-400 font-bold">[Sam · 2:18]</span> I joined in '21 — I wasn't there from the start.</p>
          <p><span className="text-violet-600 dark:text-violet-400 font-bold">[Jordan · 2:22]</span> We almost shut down in Q2.</p>
    ...
    <div className="min-h-screen bg-white dark:bg-gray-950 transition-colors duration-500">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
          ...
          >
            ...
            <p className="text-sm text-gray-500 dark:text-white/40 max-w-2xl mx-auto mb-6">
              ...
              <Link to="/guideline-format" className="text-violet-600 dark:text-violet-400 font-semibold hover:underline">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
              <Link to="/video-to-transcript">
                <motion.span
                  ...
                  className="inline-flex items-center gap-2 bg-gradient-to-r from-violet-600 to-violet-600 text-white px-7 py-3.5 rounded-xl font-semibold text-[15px] shadow-lg shadow-violet-500/25 hover:shadow-xl transition-all"
                >
                  Try VideoText free
              ...
              <Link to="/guideline-format">
                <motion.span
                  whileHover={{ scale: 1.03, y: -1 }}
                  className="inline-flex items-center gap-2 border border-violet-300 dark:border-violet-500/50 text-violet-700 dark:text-violet-300 bg-white/80 dark:bg-gray-900/80 px-6 py-3.5 rounded-xl font-semibold text-[15px] hover:bg-violet-50 dark:hover:bg-violet-950/40 transition-all"
                >
      ...
      <div className="max-w-7xl mx-auto px-6 pb-24 space-y-16">
          ...
          <div className="space-y-8">
            {FEATURE_ROWS.map((section) => (
              <div key={section.category}>
                <h3 className="text-sm font-semibold text-violet-600 dark:text-violet-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                    ...
                    <div className="min-w-[1000px]">
                      ...
                      <div className="grid grid-cols-[180px_repeat(10,1fr)] gap-1 bg-gray-50 dark:bg-gray-900 px-4 py-3 border-b border-gray-200 dark:border-white/[0.05] transition-colors duration-500">
                        ...
                        <div className="text-center px-1">
                          <span className="text-[11px] font-bold text-violet-600 dark:text-violet-400 uppercase tracking-wide">VideoText</span>
                        </div>
                        {COMPETITORS.map(c => (
                      ...
                      <div className="divide-y divide-gray-100 dark:divide-white/[0.03] bg-white dark:bg-gray-900/50 transition-colors duration-500">
                        {section.features.map((row) => (
                          <div
                            key={row.label}
                            className={`grid grid-cols-[180px_repeat(10,1fr)] gap-1 px-4 py-3.5 items-center ${row.highlight ? 'bg-violet-50/50 dark:bg-violet-500/[0.04]' : ''} transition-colors duration-500`}
                          >
        ...
        <motion.section
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="bg-gradient-to-br from-violet-600 via-violet-600 to-violet-700 rounded-3xl p-8 sm:p-12 text-white"
        >
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            <Link to="/video-to-transcript">
              <motion.span
                whileHover={{ scale: 1.03, y: -1 }}
                className="inline-flex items-center gap-2 bg-white text-violet-700 px-8 py-3.5 rounded-xl font-bold text-[15px] shadow-lg hover:shadow-xl transition-all"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                ...
                <Link
                  key={tool.path}
                  to={tool.path}
                  className="group relative overflow-hidden rounded-xl bg-gradient-to-br from-violet-50 to-violet-50 dark:from-violet-900/20 dark:to-violet-900/20 p-4 border border-violet-200/30 dark:border-violet-500/20 hover:border-violet-400/50 dark:hover:border-violet-400/50 transition-all"
                >
                  <div className="flex items-start gap-3 mb-2">
                    <span className="text-xl">{tool.icon}</span>
                    <h3 className="font-semibold text-gray-900 dark:text-white group-hover:text-violet-600 dark:group-hover:text-violet-400 transition-colors">{tool.title}</h3>
                  </div>
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
              {[
                ...
                <div key={item.desc} className="bg-gradient-to-br from-violet-50 to-violet-50 dark:from-violet-900/20 dark:to-violet-900/20 rounded-xl p-6 text-center border border-violet-200/30 dark:border-violet-500/20">
                  <div className="text-3xl font-bold text-violet-600 dark:text-violet-400 mb-2">{item.stat}</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {[
                ...
                <div key={i} className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-white/[0.08] rounded-xl p-6 hover:border-violet-300 dark:hover:border-violet-500 transition-all">
