    ...
    >
      <div className="space-y-5">
        ...
        {mode === 'size-to-bitrate' ? (
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-1.5">Target file size (MB)</label>
              ...
              <div>
                <label className="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-1.5">Audio bitrate (kbps)</label>
            ...
            <div>
              <p className="text-xs font-medium text-gray-600 dark:text-gray-400 mb-1.5">Video duration</p>
              ...
              <div className="grid grid-cols-2 gap-3 mt-2">
                {[{ label: 'Video bitrate', val: formatBitrate(videoBitrate), note: 'Set this in your encoder' }, { label: 'Total bitrate', val: formatBitrate(totalBitrate), note: `incl. ${audioBitrate} kbps audio` }].map((s) => (
                  <div key={s.label} className="rounded-xl bg-blue-50 dark:bg-blue-900/20 p-4 text-center">
                    <p className="text-2xl font-medium text-blue-700 dark:text-blue-300">{s.val}</p>
                    <p className="text-xs font-medium text-gray-700 dark:text-gray-300 mt-0.5">{s.label}</p>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-1.5">Video bitrate (kbps)</label>
            ...
            {fileSizeB > 0 && (
              <div className="rounded-xl bg-blue-50 dark:bg-blue-900/20 p-4 text-center mt-2">
                <p className="text-3xl font-medium text-blue-700 dark:text-blue-300">{formatSize(fileSizeB)}</p>
                <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mt-0.5">Estimated file size</p>
        ...
        <div className="rounded-xl border border-gray-200 dark:border-gray-600 overflow-hidden">
          <p className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide px-4 py-2 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-600">YouTube recommended bitrates</p>
            ...
            {PRESETS.map((p) => {
              ...
              return (
                <div key={p.label} className="flex justify-between items-center px-4 py-2 text-sm">
                  ...
                  <span className="font-medium text-blue-700 dark:text-blue-400">{formatBitrate(recBitrate)}</span>
                </div>
              )
            })}
          </div>
