  ...
  return (
              ...
              <div className="overflow-hidden rounded-xl border border-gray-200 dark:border-gray-600">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 dark:bg-gray-800">
                    <tr>
                      <th className="text-left px-4 py-2 text-xs font-medium text-gray-500 uppercase tracking-wide">Platform</th>
                      <th className="text-left px-4 py-2 text-xs font-medium text-gray-500 uppercase tracking-wide">Max chars/line</th>
                      <th className="text-left px-4 py-2 text-xs font-medium text-gray-500 uppercase tracking-wide">Max lines</th>
                    </tr>
                  </thead>
    ...
    >
      <div className="space-y-4">
        ...
        <div>
          <p className="text-xs font-medium text-gray-600 dark:text-gray-400 mb-2">Platform standard</p>
          <div className="grid grid-cols-4 gap-2">
            {(Object.keys(STANDARDS) as (keyof typeof STANDARDS)[]).map((s) => (
              <button key={s} onClick={() => setStandard(s)} className={`py-2 rounded-lg text-xs font-medium uppercase tracking-wide transition-colors ${standard === s ? 'bg-blue-600 text-white' : 'border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>{STANDARDS[s].name}</button>
            ))}
          </div>
          {standard === 'custom' && (
        ...
        <button onClick={handleCheck} className="w-full py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-medium text-sm transition-colors">Check Character Limits</button>
              ...
              <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1">
                {failing.map((r) => (
                  <div key={r.index} className="rounded-lg border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20 px-3 py-2 text-xs">
                    <p className="font-medium text-red-700 dark:text-red-400 mb-0.5">Cue #{r.index}</p>
