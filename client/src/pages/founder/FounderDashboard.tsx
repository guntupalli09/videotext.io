...
function KpiCard({ label, value, sub, accent = false, danger = false }: {
  ...
  return (
    <div className={`rounded-xl border p-4 flex flex-col gap-1 ${
      ...
      <p className={`text-2xl font-medium ${danger ? 'text-red-400' : accent ? 'text-blue-300' : 'text-white'}`}>{value}</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
                ...
                <div className="space-y-2 text-xs">
                  <p className="text-zinc-300">Activation: <span className="text-white font-medium">{growth.latest.activationRatePct}%</span> (target {growth.kpiTargets.activationRatePct.target}%)</p>
                  <p className="text-zinc-300">Upgrade CTR (activated): <span className="text-white font-medium">{growth.latest.activatedUpgradeCtrPct}%</span> (target ≥{growth.kpiTargets.activatedUpgradeCtrPct.target}%)</p>
                  <p className="text-zinc-300">Paid conversion (free base): <span className="text-white font-medium">{growth.latest.paidConversionPct}%</span> (target ≥{growth.kpiTargets.paidConversionPct.target}%)</p>
                </div>
              </div>
              <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
                <h3 className="text-sm font-medium text-white mb-3">Daily founder report</h3>
                <div className="grid grid-cols-2 gap-2 text-xs text-zinc-300">
                  <p>New free signups</p><p className="text-right text-white font-medium">{growth.founderDailyReport.newFreeSignups}</p>
                  <p>Users in 3–6h window</p><p className="text-right text-white font-medium">{growth.founderDailyReport.usersIn3To6hWindow}</p>
                  <p>First outputs completed</p><p className="text-right text-white font-medium">{growth.founderDailyReport.firstOutputsCompleted}</p>
                  <p>Upgrade clicks</p><p className="text-right text-white font-medium">{growth.founderDailyReport.upgradeClicks}</p>
                  <p>Paid conversions</p><p className="text-right text-white font-medium">{growth.founderDailyReport.paidConversions}</p>
                </div>
              </div>
              <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
                ...
                <div className="space-y-2 text-xs text-zinc-300">
                  <p>Day 7: before <span className="text-white font-medium">{growth.cohortComparison.day7.beforePct}%</span> → after <span className="text-emerald-300 font-medium">{growth.cohortComparison.day7.afterPct}%</span></p>
                  <p>Day 14: before <span className="text-white font-medium">{growth.cohortComparison.day14.beforePct}%</span> → after <span className="text-emerald-300 font-medium">{growth.cohortComparison.day14.afterPct}%</span></p>
                </div>
              </div>
            </div>
          </section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-0.5">
          {users.map((u, i) => (
            <div key={u.userId} className="flex items-center gap-3 py-1.5 border-b border-zinc-800/40">
              ...
              <span className="text-xs text-white font-medium tabular-nums w-10 text-right">{u.jobCount}</span>
            </div>
          ))}
        </div>
      )}
