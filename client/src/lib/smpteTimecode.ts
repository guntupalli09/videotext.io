/**
 * Client-side copy of server/src/utils/smpteTimecode.ts — kept identical
 * intentionally (pure math, zero dependencies) so client-only exports like
 * buildTxt() (zero server round-trips) can compute BITC without a network
 * call. Keep both copies in sync by hand if either changes.
 *
