...
| **MRR** (canonical, operator decision 2026-07-27) | **Contractual** recurring value of currently-active subscriptions — the subscription's committed price, **independent of any temporary invoice-level discount or promo code** — summed **one row per subscription** | `SELECT SUM(normalized_monthly_cents) FROM business_subscriptions WHERE status='active'` — `business_subscriptions` is a **current-state table** (unique key = `stripe_subscription_id`), not an append-only ledger. Implemented in `server/src/utils/stripeMrr.ts` (`computeNormalizedMonthlyCentsFromInvoiceV2`), which derives this from the subscription's `Price` object, not from `invoice.amount_paid` | none additional; explicitly forbids summing a ledger of historical invoices (the exact Phase 1 §2.1 bug); explicitly does **not** subtract active discounts — see Collected Cash below for that figure | UTC; T2, recomputed on every relevant Stripe webhook, reconciled nightly against Stripe's live subscription list (T4) | Finance | High |
| **ARR** | `MRR × 12` | Derived, same source | same as MRR | UTC; same cadence as MRR | Finance | High |
| **Collected Cash** (separate metric — NOT MRR, never summed into or substituted for it) | Actual amount collected on the most recent invoice for a subscription (`invoice.amount_paid`) — can be less than that subscription's MRR during an active discount/promo period, converges to it once the discount lapses | `SubscriptionCurrentState.lastInvoiceAmountPaidCents` (added alongside the MRR fix, Sprint 1); logged per-invoice today in the `mrr_extraction_shadow_compare` structured log as `collectedCashCents` | none additional | UTC; T2, webhook-driven | Finance | High |
