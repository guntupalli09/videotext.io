...
|---|---|---|---|---|---|
| 1 | **[REVISED 2026-07-27 — see IMPLEMENTATION_PROGRESS.md]** Originally believed: MRR computed by summing an append-only `SubscriptionSnapshot` ledger with no dedup — long-tenured subscribers counted once per renewal invoice. **Confirmed live root cause instead: `SubscriptionSnapshot.priceMonthly` has been `0` and `stripeSubscriptionId` `NULL` for every row ever written (100% of the table's history).** `computeNormalizedMonthlyCentsFromLines()`/`getPriceRecurring()` (`server/src/utils/stripeMrr.ts`) read `line.type`, `line.price`, and `line.subscription` — all three fields were removed/relocated in the Stripe API version this account is pinned to (`2026-01-28.clover`); the real data now lives at `line.parent.type`, `line.pricing.price_details.price` (a string, not an object), and `line.parent.subscription_item_details.subscription`. Confirmed via a read-only inspection of a real, paid, correctly-configured $40/mo subscription invoice — the defect is purely extraction, not the underlying Stripe data. | **Critical**, unchanged | Investor/pricing/hiring decisions made off an MRR figure that has been **$0 since this tracking began**, not inflated as originally assumed — arguably a more severe failure mode for trust in the number | **Medium** (revised up from Low) — this is a fix to webhook parsing of live revenue events, not an isolated read-only dashboard query | **8 points** (revised up from Small) — see `SPRINT_PLAN.md` Sprint 1 (revised) |
