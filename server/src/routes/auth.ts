...
/**
 * Apple's ID tokens are RS256 JWTs signed with rotating keys from:
 *   https://appleid.apple.com/auth/keys
 * We cache the JWKS in memory (1 hour TTL) to avoid hitting Apple on every request.
 */
...
async function verifyAppleIdToken(idToken: string, clientId: string): Promise<AppleIdTokenClaims> {
  ...
  if (!header.kid) throw new Error('Apple ID token missing kid')

  const keys = await getApplePublicKeys()
  const jwk = keys.find((k) => k.kid === header.kid)
  if (!jwk) throw new Error('Apple public key not found for kid: ' + header.kid)

  // Convert JWK → PEM using Node.js built-in crypto (Node 18+)
...
/** Redis key for Apple sub → userId mapping (permanent — never expires). */
function appleSubKey(sub: string): string { return `apple_sub:${sub}` }

const appleAuthLimit = rateLimit({
  windowMs: 60 * 1000,
  max: 20,
  standardHeaders: true,
  legacyHeaders: false,
  keyGenerator: (req) => (req.ip ?? 'unknown'),
  message: { message: 'Too many Apple login attempts. Please wait a minute.' },
})

/**
 * POST /api/auth/apple
 * Body: { idToken: string, email?: string }
 *   - idToken: Apple ID token from Sign in with Apple JS popup
 *   - email: user's email (ONLY present on first sign-in; must be cached on subsequent logins)
 *
 * Apple only provides the email on the very first sign-in. We store the
 * apple_sub → userId mapping in Redis so returning users can be identified
 * by their stable `sub` claim without needing the email again.
 */
router.post('/apple', appleAuthLimit, async (req: Request, res: Response) => {
  try {
    const { idToken, email: emailFromClient } = req.body as { idToken?: string; email?: string }
    ...
    if (!clientId) {
      return res.status(503).json({ message: 'Apple login is not configured on this server.' })
    }

    // Verify Apple ID token — throws if invalid/expired
    let claims: AppleIdTokenClaims
    try {
      claims = await verifyAppleIdToken(idToken, clientId)
    } catch (e) {
      log.warn({ msg: 'Apple ID token verification failed', error: (e as Error).message })
      return res.status(401).json({ message: 'Invalid Apple credential.' })
    }

    const appleSub = claims.sub

    // --- Returning user: look up by Apple sub in Redis ---
    const existingUserId = await otpRedis.get(appleSubKey(appleSub))
    if (existingUserId) {
      const user = await prisma.user.findUnique({ where: { id: existingUserId } })
      if (user) {
        const token = signAuthToken(user as unknown as User)
        log.info({ msg: 'Apple OAuth returning user login', userId: existingUserId })
        return res.json({ token, userId: user.id, plan: user.plan, email: user.email })
      }
      // User was deleted — fall through to re-create
      await otpRedis.del(appleSubKey(appleSub))
    }

    // --- First-time Apple sign-in: need email ---
    // Apple includes email in the ID token claims on first sign-in; client also passes it.
    const email = (claims.email || emailFromClient || '').toLowerCase().trim()
    if (!email || !isValidEmail(email)) {
      return res.status(400).json({
        message: 'Could not retrieve your email from Apple. Please try again or use email sign-up.',
      })
    }
    ...
    if (!user) {
      ...
      const newUser: User = {
        id: crypto.randomUUID(),
        email,
        passwordHash: randomHash, // unusable random hash — Apple is the auth provider
        plan: 'free',
        stripeCustomerId: undefined,
        subscriptionId: undefined,
        paymentMethodId: undefined,
      ...
      user = newUser
      log.info({ msg: 'Apple OAuth new user created', email })
    } else {
      log.info({ msg: 'Apple OAuth linked to existing account', email })
    }

    // Store apple_sub → userId permanently so returning users don't need email
    await otpRedis.set(appleSubKey(appleSub), user.id)

    const token = signAuthToken(user)
    return res.json({ token, userId: user.id, plan: user.plan, email: user.email })
  } catch (error: unknown) {
    log.error({ msg: 'apple-auth error', error: (error as Error)?.message ?? String(error) })
