...
function upgradeRescueHtml(ctaUrl: string): string {
  return `
<!DOCTYPE html>
<html>
...
export async function runUpgradeRescueCron(): Promise<void> {
  ...
  const intents = await prisma.upgradeIntent.findMany({
    ...
    select: { id: true, userId: true, source: true, createdAt: true, user: { select: { email: true, plan: true } } },
  ...
  for (const intent of dedupedByUser.values()) {
    ...
    const html = upgradeRescueHtml(`${baseUrl}/pricing`)
    const res = await fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${resendKey}` },
      body: JSON.stringify({ from: fromEmail, to: [email], subject: RESCUE_SUBJECT, html }),
