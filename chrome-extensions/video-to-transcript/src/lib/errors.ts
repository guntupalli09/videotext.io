...
export function isNetworkError(e: unknown): boolean {
  if (e instanceof VideoTextApiError) return false
  if (e instanceof TypeError && /failed to fetch|load failed|network/i.test(e.message)) return true
...
export function getUserFacingMessage(e: unknown): string {
  if (isNetworkError(e)) return 'Connection lost. Check your network and try again.'
