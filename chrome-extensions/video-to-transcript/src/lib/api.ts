...
/** Shape of GET /api/job/:jobId — server/src/routes/jobs.ts buildJobStatusPayload(). */
...
/** Subset of GET /api/usage/current the extension needs — server/src/routes/usage.ts. */
export interface UsageData {
  ...
  usage?: { importCount?: number; totalMinutes?: number }
  resetDate?: string
}
...
async function toApiError(response: Response, fallback: string): Promise<VideoTextApiError> {
...
export async function getCurrentUsage(token: string): Promise<UsageData> {
  const response = await request('/api/usage/current', token, { timeoutMs: GET_TIMEOUT_MS })
  if (!response.ok) throw await toApiError(response, 'Could not load your VideoText account.')
...
export async function getJobStatus(
  ...
  if (!response.ok) throw await toApiError(response, 'Could not check transcription status.')
  return (await response.json()) as JobStatus
}

/** Fetch transcript text from result.downloadUrl. Ownership is enforced server-side (server/src/routes/download.ts). */
export async function fetchTranscriptText(downloadUrl: string, token: string): Promise<string> {
  ...
  if (!response.ok) throw await toApiError(response, 'Could not download the transcript.')
  return response.text()
}

export interface UploadOptions {
  /** English language name, or '' for auto-detect. Normalised server-side. */
  language: string
  /** Set when the input is an audio file, matching the web app's audio-only upload mode. */
  audioOnly: boolean
...
function parseUploadResponse(text: string): UploadResult {
  ...
  if (!data?.jobId) throw new Error('Upload accepted but no job was created. Please try again.')
...
async function uploadSingle(
  ...
  if (!response.ok) throw await toApiError(response, 'Upload failed. Please try again.')
...
async function uploadChunked(
  ...
  if (options.language) initBody.language = options.language
  if (options.audioOnly) {
  ...
  if (!initResponse.ok) throw await toApiError(initResponse, 'Upload could not be started.')
  const { uploadId } = (await initResponse.json()) as { uploadId?: string }
  if (!uploadId) throw new Error('Upload could not be started. Please try again.')
  ...
  for (let index = 0; index < totalChunks; index++) {
    ...
    if (!chunkResponse.ok) throw await toApiError(chunkResponse, 'Upload interrupted. Please try again.')
  ...
  if (!completeResponse.ok) throw await toApiError(completeResponse, 'Upload could not be finished.')
 ...
 */
export function getJobLifecycleTransition(status: JobStatus): 'continue' | 'completed' | 'failed' {
