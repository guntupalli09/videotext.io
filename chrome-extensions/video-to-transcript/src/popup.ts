...
type View = 'signed-out' | 'idle' | 'processing' | 'result' | 'error'

const VIEW_IDS: Record<View, string> = {
...
function showIdleError(message: string | null): void {
  const node = el('idle-error')
  node.textContent = message ?? ''
  node.hidden = !message
}

function showError(message: string, options: { offerUpgrade?: boolean } = {}): void {
...
function clearSelectedFile(): void {
  ...
  el<HTMLButtonElement>('transcribe').disabled = true
  showIdleError(null)
}
...
async function selectFile(file: File): Promise<void> {
  ...
  el<HTMLButtonElement>('transcribe').disabled = false
...
function setProcessing(title: string, detail: string, fileName: string): void {
...
function setUploadProgress(percent: number | null): void {
...
async function startTranscription(): Promise<void> {
  ...
  const language = el<HTMLSelectElement>('language').value
  ...
  try {
    const result = await uploadForTranscription(file, session.token, {
      ...
      onProgress: (percent) => {
        ...
        if (file.size > 15 * 1024 * 1024) setUploadProgress(percent)
      },
    })
 ...
 * is still completed.
 */
async function pollJob(jobId: string, jobToken: string | undefined, fileName: string): Promise<void> {
  stopPolling()
  if (!session) return
  ...
  const tick = async (): Promise<void> => {
    ...
    setUploadProgress(
      status.status === 'processing' && typeof status.progress === 'number' && status.progress > 0
        ? status.progress
        : null
    )
    pollTimer = setTimeout(() => void tick(), JOB_POLL_INTERVAL_MS)
...
async function showResult(status: JobStatus, fileName: string): Promise<void> {
  if (!session) return
...
async function maybeAskForReview(): Promise<void> {
  ...
  try {
    ...
    if (shouldShowReviewPrompt(state)) prompt.hidden = false
  } catch {
...
async function resolveReviewAsk(action: 'leave' | 'dismiss'): Promise<void> {
...
function populateLanguages(): void {
  const select = el<HTMLSelectElement>('language')
...
async function init(): Promise<void> {
  ...
  await refreshUsage()
  if (!session) return // refreshUsage may have signed us out on a 401.
