...
| 19a | **Review ask appears** | Complete your first transcription on a fresh profile | Below **New Transcription**: "Finding VideoText useful? / Share your experience on the Chrome Web Store. An honest review helps other people find the extension." with **Leave a review** and **Not now** |
| 19b | **Review ask does not nag** | Dismiss it, then run two more transcriptions the same day | It does not reappear. The 2nd ask needs both the 5-job milestone **and** 7 days since the last ask |
| 19c | **Leave a review** | Click it | Opens this item's Store reviews page in a new tab; the prompt never appears again (`videotext:reviewPrompt` → `status: "asked"`) |
| 19d | **Review state resets for testing** | In the popup console: `chrome.storage.local.remove('videotext:reviewPrompt')`, then transcribe | The prompt returns on the next completed job |
